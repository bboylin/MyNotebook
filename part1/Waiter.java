...
/**
 * <p>
 * a tool used for main test thread to waits for new spawn thread to be finished ,
 * and safely perform assertion at new spawn thread.
 * </p>
 * for example：
 * <pre>
 *     final Waiter waiter = new Waiter(1);
...
public final class Waiter {
    /** assertion error AtomicReference */
    private final AtomicReference<AssertionError> mAssertionError = new AtomicReference<>(null);
    /** count down latch */
    private CountDownLatch mLatch;

    /**
     * Constructs a {@linkplain Waiter} initialized with the given count.
     *
     * @param count the number of times {@link #resume} must be invoked
    ...
    /**
     * marks a new spawn thread has finished, and causing count--
     */
    public void resume() {
        mLatch.countDown();
    }
    ...
    /**
     * await unit new spawn thread to be finished
     */
    public void await() throws InterruptedException, AssertionError {
        mLatch.await();
        if (mAssertionError.get() != null) {
            throw mAssertionError.get();
        }
    }
    ...
    public void fail(String message) {
        if (message == null) {
            mAssertionError.set(new AssertionError());
        } else {
            mAssertionError.set(new AssertionError(message));
        }
    }
    ...
    public void assertEquals(String message, Object expected,
        ...
        if (!equalsRegardingNull(expected, actual)) {
            if (expected instanceof String && actual instanceof String) {
                ...
                mAssertionError.set(new ComparisonFailure(cleanMessage, (String) expected,
                        (String) actual));
            } else {
